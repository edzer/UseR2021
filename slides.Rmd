...
* For small area's, projection is usually no big issue (Earth is nearly flat)
...
a while due to doing it wrong for some 50+ years.
...
Platform](https://openeo.org/platform/) are a larger initiatives for
allowing reproducible (open source) computing on large, cloud-based
...
* R Spatial is an open and friendly community of people using R for handling and analysing spatial data
* It uses and interfaces a lot of software shared with a much larger community (e.g. OSGEO)
* `rgdal` and `rgeos` will retire Jan 1, 2024; `sf`, `terra`, `stars` provide alternatives
* `sf` now uses spherical geometry:
    * straight lines may need noding, 
    * simplifying should be done _after_ projection
